...
void VideoManager::init(string ip, boost::asio::io_service &io_service)
{
	...
	tcp::resolver::query query(ip, to_string(ardrone2::VIDEO_PORT));
				...
				else
				{
					...
					if(frameAvailable)
					{
						...
						if(_recording_rawFrame != nullptr && _recording_pave != nullptr)
						{
							...
							if(_recording_pave->frame_type == ardrone2::video::frame_type::I || _recording_pave->frame_type == ardrone2::video::frame_type::IDR)
							{
...
void VideoManager::decodePacket()
{
	...
	if((_pave->frame_type == ardrone2::video::frame_type::I || _pave->frame_type == ardrone2::video::frame_type::IDR))
	{
	...
	if(got_first_iframe) // Wait for I-frame
	{
		...
		if(_pave->video_codec == ardrone2::video::codec::MPEG4_VISUAL)
		{
			avcodec_decode_video2(cfg.pCodecCtxMP4, cfg.pFrame, &frameFinished, &_packet);
		}
		else if(_pave->video_codec ==  ardrone2::video::codec::MPEG4_AVC)
		{
...
unsigned long VideoManager::decodedPackets()
{
	return _decodedPackets;
}
